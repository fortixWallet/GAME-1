...
# 0в) ПОКАЗАНО ≠ НАПИСАНО (правило 22): кожен факт несе shown_in; голих не
#     більше, ніж у зафіксованому списку лікування; битих посилань на арт — нуль.
if command -v python3 >/dev/null 2>&1; then
  SH=$(python3 tools/audit_shown.py 2>&1); SHRC=$?
  echo "--- audit_shown ---" >>"$LOG"; echo "$SH" >>"$LOG"
  echo "$SH" | grep "^SHOWN_AUDIT "
  if [ $SHRC -ne 0 ]; then
    echo "ПРАВИЛО 22 ПОРУШЕНО:"; echo "$SH" | grep -E "БИТИЙ|FAIL"
    exit 2
  fi
fi
