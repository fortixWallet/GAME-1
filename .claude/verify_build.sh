...
for t in "$@"; do
  ...
  case "$t" in
    ...
    "furnprobe") echo "$OUT" | grep -qE "C2PROBE_OK bad_screen=0 bad_frame=0 no_exit=0 facts=16/16" || FAIL="$FAIL c2probe" ;;
    # walk q: записник гортається; 15 фактів справи 2 розкладено по аркушах
    "walk q")   echo "$OUT" | grep -q "WALK_Q_OK rows_first=10 rows_last=6 total=16" || FAIL="$FAIL walk-q" ;;
