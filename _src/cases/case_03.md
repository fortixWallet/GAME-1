...
## 14. ЕТАЛОН-ПАС 04.08 (правила 20 · 22 · 24 · 25 · 28 — справа стає еталонною)

**МЕДІУМ (правило 20, обрано раз):** 2D-плити 1200×896 + HD 2x, як скринька.
3D зі спеки СКАСОВАНО (урок секретера; Meshy дрібний механізм не потягне).
Розбирання — станами однієї камери: `wa_closed` (майстер: кришкою догори,
дужка звисає за край столу у профіль) → `wa_back_open` (нутро кришки: клейма
13·A·64, JW у ромбі, 4198) → `wa_back_raked` (косе світло: 6 рядків подряпин)
→ `wa_cuvette` (кювета: Piguet-Rey 21607, ДВІ ЗАГЛУШЕНІ ДІРКИ ключа) →
`wa_movement` (платина: Cuenot 118744 + ЛАТУННЕ КІЛЬЦЕ в гнізді).
Наближення (дужка, проріз ранта) — ТІЛЬКИ кропом майстра/станів.

**ЧИСЛА ГЕТЬ (Р7 + правило 24):** обидві NUMBER-графи скасовано.
- `s.bureau_year` → графа заповнюється ПРИКОЛЮВАННЯМ фішки «квитанція 1881»;
  рік пише гра.
- `s.assembled` → гравець приколює ПІДСТАВУ: фішка «список номерів механізму»
  → гра пише «не раніше 1888» (найкраща) · фішка «подряпина 9/93» → «не
  раніше 1893» (прийнятна, гірша — гілка out.dated_late). Стрибок думки
  живе у ВИБОРІ підстави, не у введенні числа. Номери 4198/21607/118744
  гравець НЕ запам'ятовує ніколи: їх носять фішки.

**ПОКАЗАНО (правило 22) — shown_in кожного факту:**
| факт | кадр, де ВИДНО |
|---|---|
| f.case_marks | wa_back_open (+HD під лупу) |
| f.scratch_dates | wa_back_raked — подряпини ВИДНІ як рядки |
| f.cuvette_number · f.cuvette_plugs | wa_cuvette — заглушки читаються оком |
| f.movement_number | wa_movement |
| f.spacer | wa_movement — кільце СВІТИТЬСЯ свіжим точінням |
| f.band_cut | кроп wa_closed: білі сліди напилка в прорізі |
| f.bow_saddle | кроп wa_closed: дужка у профіль, дві борозни |
| f.tbl_* · f.deed/list/letter/receipt | папери — читаються (виняток 22) |
Клейма в арті книги клейм — перемалювати з реальної таблиці
(925-1000.com/Faustria_02.html), без копіювання зображення (IP).

**ЧАСТКОВЕ ПІДТВЕРДЖЕННЯ (правило 25):** графа з вірним значенням І вірними
фішками темніє чорнилом одразу. Невірна не коментується (помилка = подія
тільки на печатці).

**ЗВІРЕНО ВЕБОМ (правило 12):** 13 лотів (Wiener Probe) · A=Wien · злам
системи 1866→товсотні/Діана: silvercollection.it/austriansilverhallmarks.html,
925-1000.com/Faustria_menu.html. Заглушені дірки ключа при переробці на
голівку, перехідні механізми 1880-х: mb.nawcc.org/threads/179046,
watchmuseum.org/key-wind-vs-stem-wind. Ligne 2.2558 мм — звірити при
виробництві книги (не тримає загадку).

**ПРИЙМАННЯ (правило 13 + 28):** еталон-кадр wa_closed — «оце воно» Віктора
ДО серії. Після збірки — свіжий казуал кліками формулює своїми словами
цитату з §13. Без цього справа не здана.
