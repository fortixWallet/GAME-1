...
const ZONES := {
	...
	&"z.void.lining": {
		"kind": &"img", "screen": &"C2OPEN", "surface": &"box_noboard",
		"u": Vector2(0.595, 0.265), "shape": &"rect", "half": Vector2(0.045, 0.185),
...
const RULES := [
	...
	{"zone": &"z.well.back_board", "tool": &"tool.loupe", "requires": [&"f.board_screwed"],
	 ...
	 "say": "The screws run to a sharp point. The thread is even from head to tip. A screw of this make has a birthday — the screw-book among the papers will give it."},
	{"zone": &"z.well.back_board", "tool": &"tool.loupe", "requires": [&"f.screw_points"],
	 "facts": [&"f.slot_burr"],
	 "say": "Glass from head to head: three slots bright and torn, their wax cracked in a ring. The fourth stands a hair proud, its wax whole."},
	...
	{"zone": &"z.screws.loose", "tool": &"tool.loupe",
	 "say": "Under the glass: three heads chewed by a hurried driver; the fourth clean, its wax unbroken — never turned until today."},
...
const FACTS := {
	...
	&"f.syn_late_screws":      {"cite": "the board is held by screws younger than the box", "tag": &"books", "weight": 3,
